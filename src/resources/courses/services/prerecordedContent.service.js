...
import { uploadVideo, uploadImage, uploadDocument, deleteFile, generateThumbnail } from "../../../utils/image/cloudinary.js";
...
class PrerecordedContentService {
  ...
  async updateVideoClass(classId, updateData, instructorId) {
    try {
      ...
      const instructor = await User.findById(instructorId);
      if (videoClass.instructor.toString() !== instructorId && instructor.role !== "super admin") {
    ...
    try {
      ...
      for (const resource of resources) {
        if (resource.file.publicId) {
          ...
          await deleteFile(resource.file.publicId, resourceType === 'video' ? 'video' : resourceType === 'image' ? 'image' : 'raw');
        }
  ...
  async uploadClassResource(resourceData, file, uploadedBy) {
    try {
      ...
      if (resourceData.classType === "PrerecordedClass") {
        ...
        const LiveClass = (await import("../../live-classes/models/liveClass.js")).default;
      ...
      const uploadOptions = {
        ...
        resource_type: fileType === 'video' ? 'video' : fileType === 'image' ? 'image' : 'raw'
      };
  ...
  async downloadResource(resourceId, userId) {
    try {
      ...
      if (resource.accessLevel === "enrolled_only") {
        ...
        if (!enrollment) {
          throw new Error("You must be enrolled in this course to download this resource");
        }
      }
  ...
  async deleteClassResource(resourceId, userId) {
    try {
      ...
      if (resource.uploadedBy.toString() !== userId && user.role !== "super admin") {
