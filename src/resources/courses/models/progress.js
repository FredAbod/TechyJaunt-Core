...
// Virtual for backward compatibility with UserCourseProgress status
progressSchema.virtual("status").get(function () {
  if (this.isCompleted) return "completed";
  if (this.overallProgress > 0) return "in-progress";
  return "enrolled";
});
...
progressSchema.methods.updateLessonProgress = function (
  moduleIndex,
  lessonId,
  watchTime,
  totalDuration,
...
progressSchema.methods.isModuleCompleted = function (moduleIndex) {
  ...
  const hasPassedAssessment = module.assessmentAttempts.some(
    (attempt) => attempt.passed,
  );

  return (
    allLessonsCompleted &&
    (hasPassedAssessment || module.assessmentAttempts.length === 0)
  );
};

// Method to unlock next module
progressSchema.methods.unlockNextModule = function () {
  const currentModuleCompleted = this.isModuleCompleted(
